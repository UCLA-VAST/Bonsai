...
int main(int argc, char** argv)
{
    ...
    // sorted chunk increase 2^4 per each pass 
    uint8_t num_pass = (uint8_t) ((pow_num - 4) % 4) == 0 ? ((pow_num - 4) / 4) : ((pow_num - 4) / 4) + 1;
...
//OPENCL HOST CODE AREA START
    ...
    OCL_CHECK(err, cl::Kernel krnl_sorter(program,"merger_tree_p8_l8_i16", &err));
...
//OPENCL HOST CODE AREA END
    ...
    for (i = 0; i < total_words/4096; i++) {
        for (j = 0; j < 4096 - 1; j++) {
            if(h_input[i*4096+j] > h_input[i*4096+j+1]) {
                printf("%ld element %08x is larger than %ld element %08x\n", i*4096+j+1, h_input[i*4096+j], i*4096+j+2, h_input[i*4096+j+1]);
                check_status = 1;
                goto printinfo;
            }
        }
